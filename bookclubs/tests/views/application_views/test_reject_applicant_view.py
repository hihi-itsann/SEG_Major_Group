...
class AcceptApplicantViewTestCase(TestCase):
    """Tests for the rejection of an application"""

    VIEW = 'reject_applicant'
    ...
    def test_reject_applicant_url(self):
        self.assertEqual(self.url, f'/club/{self.club.club_name}/reject/{self.applicant.id}/')
    ...
    def test_reject_applicant_redirects_when_not_a_member(self):
        self.log_in(self.applicant)
        redirect_url = reverse('feed')
        response = self.client.get(self.url)
        self.assertRedirects(response, redirect_url, status_code=302, target_status_code=200)

    def test_reject_applicant_redirects_when_a_member(self):
        self.log_in(self.applicant)
    ...
    def test_reject_applicant_is_successful_when_owner(self):
        ...
        self.assertEqual(after_count_members, before_count_members)
