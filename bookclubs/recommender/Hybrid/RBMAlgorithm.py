...
import numpy as np
from bookclubs.recommender.Hybrid.RBM import RBM
...
class RBMAlgorithm(AlgoBase):
    ...
    def __init__(self, epochs=20, hiddenDim=100, learningRate=0.001, batchSize=100, sim_options={}):
        ...
        self.ml = BookLens()
        self.ml.loadBookLensLatestSmall()
    ...
    def buildStoplist(self, trainset):
        ...
        for iiid in trainset.all_items():
            ...
            title = self.ml.getBookTitle(movieID)
            if (title):
                title = title.lower()
    ...
    def fit(self, trainset):
        ...
        #print(len(list(trainset.all_ratings())))
        trainingMatrix = np.zeros([numUsers, numItems, 20], dtype=np.float32)
        for (uid, iid, rating) in trainset.all_ratings():
            if not self.stoplistLookup[iid]:
                #print(rating)
                adjustedRating = int(float(rating)*2.0) - 1
                #print(adjustedRating)
