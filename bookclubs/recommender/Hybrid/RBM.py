...
class RBM(object):
    ...
    def GetRecommendations(self, inputUser):
        ...
        hidden = tf.nn.sigmoid(tf.linalg.matmul(self.X, self.weights) + self.hiddenBias)
        visible = tf.nn.sigmoid(tf.linalg.matmul(hidden, tf.transpose(self.weights)) + self.visibleBias)
    ...
    def MakeGraph(self):
        ...
        hProb0 = tf.nn.sigmoid(tf.linalg.matmul(self.X, self.weights) + self.hiddenBias)
        # Sample from all of the distributions
        hSample = tf.nn.relu(tf.math.sign(hProb0 - tf.random.uniform(tf.shape(hProb0))))
        # Stitch it together
        forward = tf.linalg.matmul(tf.transpose(self.X), hSample)

        # Backward pass
        # Reconstruct visible layer given hidden layer sample
        v = tf.linalg.matmul(hSample, tf.transpose(self.weights)) + self.visibleBias

        # Build up our mask for missing ratings
        vMask = tf.math.sign(self.X) # Make sure everything is 0 or 1
        vMask3D = tf.reshape(vMask, [tf.shape(v)[0], -1, self.ratingValues]) # Reshape into arrays of individual ratings
        vMask3D = tf.math.reduce_max(vMask3D, axis=[2], keepdims=True) # Use reduce_max to either give us 1 for ratings that exist, and 0 for missing ratings
        ...
        hProb1 = tf.nn.sigmoid(tf.linalg.matmul(vProb, self.weights) + self.hiddenBias)
        backward = tf.linalg.matmul(tf.transpose(vProb), hProb1)
